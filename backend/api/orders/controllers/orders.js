 ...
 * @param number
 */
const fromDecimalToInt = (number) => parseInt(number * 100); //stripe samo int prima
...
module.exports = {
  ...
  async create(ctx) {
    ...
    const { products } = ctx.request.body;
    if (!products) {
      return res.status(400).send({ error: "Please add a products" });
    }
    ...
    for (let i = 0; i < products.length; i++) {
      let realProduct = await strapi.services.products.findOne({
        id: products[i].id,
      });
      if (!realProduct) {
  ...
  async confirm(ctx) {
    ...
    if(session.payment_status === "paid"){
        ...
        ctx.throw(400, "It seems like the order wasn't verified!");
    }
}
